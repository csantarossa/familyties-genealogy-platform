...
const treeId = parseInt(context.params.id);
...
for (const [gedcomId, person] of Object.entries(individuals)) {
    ...
    if (!birthISO && person.birth) notes.push(`Unparsed birth date: ${person.birth}`);
    if (!deathISO && person.death) notes.push(`Unparsed death date: ${person.death}`);
...
for (const fam of Object.values(families)) {
    const husbandId = gedcomToDbId[fam.husband];
    const wifeId = gedcomToDbId[fam.wife];
    const children = fam.children?.map(c => gedcomToDbId[c]).filter(Boolean) || [];
    ...
    if (husbandId && wifeId) {
        ...
        VALUES (${husbandId}, ${wifeId}, 3), (${wifeId}, ${husbandId}, 3)
        `;
    }
